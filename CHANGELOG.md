...
- **Since then** (0.37.9-0.37.18): scarves and snacks describe themselves
  in the PACK; the MASTER challenge's ending is finished properly; and the
  README is rewritten with screenshots of everything, credits for every
  artist, and the challenge behind a spoiler fold with its full account in
  `docs/MASTER_CHALLENGE.md`.

New in 0.37.29: the tester's checklist moves out of the download and
into the repository's docs, behind a spoiler note. On Gold and Silver, Cianwood no longer has a second post
...
**Gold and Silver get their own Goldenrod hall front.**
...
plants, and a wooden stage with pillars and benches.
...
as a starting point, ready to be redecorated. Contests still run in
Goldenrod only; wiring Ecruteak up to run them is the next piece.

New in 0.34.37: no player-visible change -- 0.34.36's boot check was
failing and is fixed.

New in 0.34.36: ECRUTEAK CITY has a Contest Hall. The building is on the
street west of the Dance Theater and you can walk in through its door.
The city's own sign moved to a new post so it can still be read, and a
second sign names the hall. A patch of grass by the hall holds GROWLITHE
and GIRAFARIG by day, MISDREAVUS and STANTLER at night, and -- very
rarely, at any hour -- an EEVEE.

New in 0.34.35: trophies. Your first contest win, and a win at MASTER
rank in each of the five categories, each unlock a trophy for the
Trophy Case mod (once it catalogues them; nothing shows without it).

New in 0.34.34: the JUDGE has a voice -- after every appeal a verdict
line says how it landed (astonished, impressed, a nod, unmoved, a
frown) -- and after each turn you are told who goes first next, so
"appeals earlier next turn" effects are visible.

New in 0.34.33: a results ceremony. When the last appeal is done the
four score bars fill to their totals, the winner's row lights up with
the applause, the rows fall into finishing order, and then the placings
are read -- A skips the fill. And the cast's lines now live in their own
file (cast_lines.lua) with contexts: what a coordinator says in the
queue, on stage, and after you beat them or they beat you, and what the
crowd says -- with per-character and per-class voices to come from the
dialogue order.
...
the three you just beat, and the stage cast is redrawn for each new
contest.

New in 0.34.30 (a code review of the last fifteen versions):
